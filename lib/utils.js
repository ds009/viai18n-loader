...
function insert$t(defaultLang, source) {
  ...
  if (script) {
    const defaultObject = matchDefaultObject(script)
    if (defaultObject) {
      return source.replace(defaultObject[0], 'const $defaultObject = ' + defaultObject[1] + '\n' + modifiedExport)
    } else {
      return source.replace(script, script + simpleExport)
    }
  } else {
    return source + '<script>' + simpleExport + '</script>'
  }
}
...
function matchComputedProps(source) {
  const matched = source.match(/computed\s*:\s*{([\s\S]*)}/)
